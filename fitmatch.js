...
async function loadData() {
  ...
  if (sportsResult.error) {
    console.error("Ошибка загрузки видов спорта:", sportsResult.error);
...
function createCoachCard(coach) {
  ...
  card.innerHTML = `
    ...
    <div class="coach-avatar">${esc(initials(coach.name))}</div>
...
function renderSports() {
  const containers = ["homeSports", "sportsList"];
  ...
  const selects = ["sportFilter", "matchSport", "coachSport"];
  ...
  selects.forEach(function (id) {
    ...
    if (id === "sportFilter") {
      select.innerHTML = `<option value="">Все виды спорта</option>`;
    }
    if (id === "matchSport") {
      select.innerHTML = `<option value="">Не выбрано</option>`;
    }
    if (id === "coachSport") {
      select.innerHTML = `<option value="">Выбрать спорт</option>`;
    }

    sports.forEach(function (sport) {
      select.add(new Option(sport.name, sport.id));
    });

    if ([...select.options].some(function (option) { return option.value === previousValue; })) {
...
function renderCoaches() {
  ...
  filtered.forEach(function (coach) {
    container.appendChild(createCoachCard(coach));
  });
}
...
function renderRanking() {
  ...
  const ranking = [...coaches].sort(function (a, b) {
  ...
  if (!ranking.length) {
    ...
    empty.textContent = "Рейтинг пока пуст.";
...
function populateGoals() {
  ...
  const goals = [...new Set(coaches.map(function (coach) { return coach.goal; }).filter(Boolean))];

  goals.forEach(function (goal) {
    select.add(new Option(goal, goal));
  });
...
async function openProfile(coachId) {
  const coach = coaches.find(function (item) {
    return String(item.id) === String(coachId);
  });
  ...
  profileContent.innerHTML = `
    <div class="match-box">
      <p class="lead">Загрузка профиля...</p>
    </div>
  `;
  ...
  if (result.error) {
    profileContent.innerHTML = `
      <div class="match-box">
        <p class="error">Не удалось загрузить отзывы: ${esc(result.error.message)}</p>
      </div>
    `;
    return;
  }
    ...
    ? reviews.map(function (review) {
        return `
          <div class="review">
            ...
            <p>${esc(review.text || "Без текста")}</p>
          </div>
        `;
      }).join("")
  ...
  profileContent.innerHTML = `
    <div class="coach-card">
      ...
      <div class="tags">
        ...
        <span class="tag">${Number(coach.reviews_count || 0)} отзывов</span>
        <span class="tag">${Number(coach.clients_count || 0)} клиентов</span>
      </div>
  ...
  reviewForm.addEventListener("submit", async function (event) {
    ...
    if (!currentUser) {
      showMessage("reviewMessage", "Сначала войди в аккаунт.", true);
      return;
    }
    ...
    if (response.error) {
      if (response.error.code === "23505") {
        showMessage("reviewMessage", "Вы уже оставляли отзыв этому тренеру.", true);
      } else {
        showMessage("reviewMessage", response.error.message, true);
      }
      return;
    }
...
function findMatch() {
  ...
  container.innerHTML = `<h2>Лучшие совпадения</h2>`;

  if (!matches.length) {
    container.innerHTML += `<p class="lead">Пока нет доступных тренеров.</p>`;
    return;
  }
...
async function refreshUser() {
  ...
  if (response.error) {
    console.warn("Auth:", response.error.message);
    currentUser = null;
  } else {
  ...
  button.onclick = async function () {
    if (currentUser) {
      ...
      if (signOutResponse.error) {
        showMessage("authMessage", signOutResponse.error.message, true);
        return;
      }
...
function initAuthForm() {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (response.error) {
      showMessage("authMessage", response.error.message, true);
      return;
    }
    ...
    showMessage("authMessage", "Вход выполнен.");
...
function initSignup() {
  ...
  button.addEventListener("click", async function () {
    ...
    if (!email || !password) {
      showMessage("authMessage", "Введи email и пароль.", true);
      return;
    }
    ...
    showMessage(
      "authMessage",
      response.error ? response.error.message : "Регистрация выполнена. Проверь email.",
...
function initForgotPassword() {
  ...
  button.addEventListener("click", async function () {
    ...
    if (!email) {
      showMessage("authMessage", "Сначала введи свой email.", true);
      return;
    }
    ...
    showMessage("authMessage", "Письмо для восстановления пароля отправлено. Проверь email.");
  });
}
...
function initResetPassword() {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (password.length < 6) {
      showMessage("resetMessage", "Пароль должен содержать минимум 6 символов.", true);
      return;
    }
    ...
    if (response.error) {
      showMessage("resetMessage", response.error.message, true);
      return;
    }
...
function initCreateCoach() {
  ...
  button.addEventListener("click", async function () {
    ...
    if (!currentUser) {
      toggleModal("modal", true);
      showMessage("authMessage", "Сначала войди или зарегистрируйся.", true);
      return;
    }
...
function initCoachForm() {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (!currentUser) {
      showMessage("coachMessage", "Необходимо войти в аккаунт.", true);
      return;
    }
    ...
    if (!name || !sport || !period) {
      showMessage("coachMessage", "Заполни обязательные поля.", true);
      return;
    }

    if (!Number.isFinite(price) || price < 0) {
      showMessage("coachMessage", "Укажи корректную цену.", true);
      return;
    }

    const response = await supabase.from("coaches").insert({
      user_id: currentUser.id,
      name,
      sport,
      goal,
    ...
    if (response.error) {
      if (response.error.code === "23505") {
        showMessage("coachMessage", "У тебя уже есть профиль тренера.", true);
      } else {
        showMessage("coachMessage", response.error.message, true);
      }
      return;
    }

    showMessage("coachMessage", "Профиль тренера создан.");
    await loadData();
...
function initNavigation() {
  ...
  if (chooseSportBtn) {
    chooseSportBtn.addEventListener("click", function () {
      showPage("sports");
    });
  }
}
...
function initFilters() {
  const sportFilter = document.getElementById("sportFilter");
  if (sportFilter) {
    sportFilter.addEventListener("change", renderCoaches);
  }

  const formatFilter = document.getElementById("formatFilter");
  if (formatFilter) {
    formatFilter.addEventListener("change", renderCoaches);
  }
...
function initModals() {
  ...
  if (coachCancel) {
    coachCancel.addEventListener("click", function () {
      toggleModal("coachModal", false);
    });
  }
...
async function init() {
  ...
  try {
    ...
    if (container) {
      container.innerHTML = `
        <p class="error">Ошибка подключения к Supabase. ${esc(error.message || error)}</p>
      `;
    }
  }
}
